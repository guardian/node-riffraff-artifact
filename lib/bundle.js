...
const fileOrDirectory = (path) => new Promise((resolve, reject) => {
    fs_1.lstat(path, (err, stats) => {
        ...
        console.warn(`${path} must either be a file or directory`, stats.isSymbolicLink() && `(it's a symlink 🔗)`, stats.isBlockDevice() && `(it's a block device 💾)`, stats.isSocket() && `(it's a socket 🔧)`);
        resolve(false);
    });
});
